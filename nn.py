...
class NeuralNet:
    def __init__(self, inputs, labels, input_nodes, hidden_nodes):
        ...
        #self.bias = []
        self.weights_in = np.random.rand(self.inputs.shape[1],input_nodes)

        # self.weights_hidden = np.array([[np.random.uniform(
        #     0.1, 0.9) for i in range(hidden_nodes)]])
        self.weights_hidden= np.random.rand(hidden_nodes,1)
    ...
    def forward(self):
        self.hl = self.relu(np.dot(self.inputs, self.weights_in))
        self.predictions = self.relu(np.dot(self.hl, self.weights_hidden))
