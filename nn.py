...
class NeuralNet:
    def __init__(self, inputs, labels, input_nodes, hidden_nodes):
        ...
        #self.bias = []
        self.weights_in = np.random.randn(
            self.inputs.shape[1], input_nodes)*np.sqrt(2/input_nodes)
        ...
        self.weights_hidden = np.random.randn(
            hidden_nodes, 1)*np.sqrt(2/hidden_nodes)
    ...
    def forward(self):
        ...
        self.predictions = self.relu(
            np.dot(self.hl, self.weights_hidden)+self.bias_hidden)
    ...
    def backprop(self):
        ...
        del_hidden = 2*(self.labels-self.predictions) * \
            self.derivative_relu(self.predictions)
        d_hidden = np.dot(
        ...
        self.bias_in += self.alpha_bias * \
            np.sum(del_in, axis=0)/self.inputs.shape[0]
        self.weights_hidden += self.alpha * d_hidden
        self.bias_hidden += self.alpha_bias * \
            np.sum(del_hidden, axis=0)/self.inputs.shape[0]
